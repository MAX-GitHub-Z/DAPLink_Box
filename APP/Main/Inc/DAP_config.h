...
/// 命令与响应数据的最大包大小。
/// 此配置设置用于优化与调试器的通信性能，并取决于 USB 外设。通常的值为：全速 USB HID 或 WinUSB 为 64，高速 USB HID 为 1024，高速 USB WinUSB 为 512。
...
/// 命令和响应数据的最大包缓冲区。
/// 此配置设置用于优化与调试器的通信性能，并取决于 USB 外设。对于内存或 USB 缓冲区有限的设备，该设置可以降低（有效范围为 1 至 255）。
#define DAP_PACKET_COUNT        1U              ///< Specifies number of packets buffered.

/// Indicate that UART Serial Wire Output (SWO) trace is available.
/// This information is returned by the command \ref DAP_Info as part of <b>Capabilities</b>.
//SWO支持串口 但是需要实现相关实例，暂不支持 后续有时间在搞
...
/// 测试域计时器的时钟频率。计时器的值可通过 \ref TIMESTAMP_GET 函数获取。
...
#define _PIN_MASK_(n)  					GPIO_PIN_##n
#define  PIN_MASK(n) 						_PIN_MASK_(n)
...
__STATIC_FORCEINLINE uint32_t PIN_SWCLK_TCK_IN  (void) {
  return (uint32_t)((PIN_SWCLK_TCK_PORT->ODR & PIN_SWCLK_TCK_MASK) ? 1 : 0);
}
...
__STATIC_FORCEINLINE uint32_t PIN_SWDIO_TMS_IN  (void) {
  return (uint32_t)((PIN_SWDIO_TMS_PORT->IDR & PIN_SWDIO_TMS_MASK) ? 1 : 0);
}
